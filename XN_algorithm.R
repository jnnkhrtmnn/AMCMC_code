...
####################################################################################################################
################### XN ALGORITHM INSPIRED BY Xiang Neal 2014 #######################################################


run_XN <- function(n=10000, x0=as.vector(c(0,0)), C0=diag(2), S=2000, target.dens, ...){
  ...
  for (i in 2:n){
    ...
    if(U[i] < alpha){
      X[,i] <- Y
      accpt.binar[i] =  1
        if(i > 1000 & i<=S){k = k + 3*k/(100*sqrt(i))}
    } else {
      X[,i] <- X[,i-1]
      accpt.binar[i] =  0
        if(i > 1000 & i<=S){k = k - k/(100*sqrt(i))}
    }
